...
app.post('/gerar-pdf', (req, res) => {
  ...
  doc.fontSize(14).font('Helvetica').text('oportunidade que foi autuado pelo suposto cometimento da infração prevista no Art. 244, inciso I do Código de Trânsito Brasileiro.', { align: 'left' });
  doc.moveDown();
  ...
  doc.moveDown();
  oc.fontSize(14).font('Helvetica').text('Conforme determina o art. 280 do Código de Trânsito Brasileiro, o auto de infração de trânsito é documento essencial para início do processo administrativo para aplicação da penalidade de multa, motivo pelo qual sua juntada é imprescindível.',{ align: 'left' });
  doc.moveDown();
...
doc.fontSize(15).font('Helvetica-Bold').text('III.II DO RECONHECIMENTO DE OFÍCIO DA DECADÊNCIA DO DIREITO DE PUNIR O(A) CONDUTOR(A)', { align: 'left' });
  ...
  doc.fontSize(14).font('Helvetica').text('A titulo de exemplo verifica-se que o art. 244, inciso I do CTB, com base na observância do MBFT, prevê no campo “Quando Autuar” a possibilidade de imputação de penalidade em várias possibilidades, ou seja, condutar distintas.', { align: 'left' });
  doc.moveDown();
